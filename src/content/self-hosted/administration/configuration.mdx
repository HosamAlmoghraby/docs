...
We have custom guides for:
- [Okta](/docs/self-hosted/guides/okta)
- [Azure](/docs/self-hosted/guides/azure)
...
Okteto integrates with different cloud providers to store the [Okteto Registry](/docs/cloud/registry/) images.

> The sensitive part of the credentials are not included in the configuration file. Instead, it is provided to Okteto via [a secret](/docs/self-hosted/install/deployment/#cloud-provider-secret).
...
When using `byo` you'll need to [configure your registry](/docs/self-hosted/administration/configuration/#registry) to use the file system for storage.
...
The jobs that deploy your [development environments from Git](/docs/cloud/deploy-from-git/).
...
- `runner`: to configure your [custom installer image](/docs/self-hosted/administration/custom-installer-image/).
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](/docs/self-hosted/administration/configuration/#ingress) section.
...
- `repository`: The [Helm repository](/docs/cloud/deploy-from-helm/) available for every developer. This is disabled by default. To enable, set the `applications.repository` property to your Helm registry URL
...
There is only one exception where this storage class is overwritten. In case of having [volume snapshots feature](/docs/self-hosted/administration/volume-snapshots/) configured, if a storage class is required for the snapshots that storage class will have preference.
...
If this happens, [follow these instructions](/docs/self-hosted/install/troubleshooting/#private-image-registry-credentials-misconfiguration) to recover from this state.
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](/docs/self-hosted/administration/configuration/#nginx-ingress).
